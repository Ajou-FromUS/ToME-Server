...
async def update_user_mission_by_id(mission_id: int, request: Request, db: Session = Depends(get_db), token: str = Depends(verify_token)):
    update_data = await request.json()

    res = user_mission_view.update_user_mission_by_id(mission_id=mission_id, update_data=update_data, db=db, token=token)
    return res
