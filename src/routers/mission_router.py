...
async def create_mission(request: Request, db: Session = Depends(get_db)):
    ...
    res = mission_view.create_mission(mission_data, db)
    return res
...
def get_all_mission(request: Request, db: Session = Depends(get_db)):
    res = mission_view.get_all_mission(db)
    return res


# 특정 미션 조회를 위한 API
@mission.get("/{mission_id}")
def get_mission_by_id(mission_id: int, request: Request, db: Session = Depends(get_db)):
    res = mission_view.get_mission_by_id(mission_id, db)
    return res
...
async def update_mission(mission_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    res = mission_view.update_mission_by_id(mission_id, update_data, db)
    return res
...
async def delete_mission(mission_id: int, request: Request, db: Session = Depends(get_db)):
    res = mission_view.delete_mission_by_id(mission_id, db)
    return res
